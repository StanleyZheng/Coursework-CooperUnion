...
void graph::dijkstra(string start_vertex){
    ...
    v->dist = 0;
    v->path = v;
    heap myHeap(24989);             //initialize with smallest prime in hash::getPrime
    
    //variables to hold the 'return' values of heap::deleteMin(&temp_id, &temp_value, &temp_data)
    string temp_id;
    int temp_value;
    vertex *temp_data;
    ...
    //set the distance from starting vertix, path,
    while(1){
        v->known = true;
        ...
        for(list<vertex::edge>::iterator iter = v->adjList.begin(); iter != v->adjList.end(); iter++){
            ...
            if (adjacentv->known == false){
                if (adjacentv->dist == -1)
                    adjacentv->dist = v->dist + iter->cost;
                else if (iter->cost + v->dist < adjacentv->dist){
                    adjacentv->dist = iter->cost + v->dist;
                }
                else //if cost + dist >= adjacent's dist, do nothing/move on
                    continue;
        ...
        do {
            if (myHeap.deleteMin(&temp_id, &temp_value, &temp_data) == 1)
                return;
        } while (temp_data->known);

        temp_data->dist = temp_value;
        v = temp_data;
    }
    return;
}
...
void graph::printPaths(string start_vertex, ofstream &output){
    ...
    for(list<vertex>::iterator iter = data.begin(); iter != data.end(); iter++){
        ...
        string temp = iter->id;
        while(iter->path != v){
            temp = iter->path->id + ", " + temp;
            iter->path = iter->path->path;
        }
        temp = v->id + ", " + temp;
        output << temp << "]" << endl;
    }
